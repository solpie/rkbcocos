...
export default class PlayerBar4v4 extends InjectCls {
  ...
  start() {
    ...
    _c_.on(EVENT_PLAYER_BAR_4V4, (data) => {
      ...
      let p = data[this.idx];
      if (p) {
        if (p.blood < 0) p.blood = 0;
        this.bloodBar.x =
          this.bloodBarInitX - (p.blood / p.init_blood) * BIG_BLOOD_BAR_WIDTH;
  ...
  test() {
    _c_.emit(EVENT_PLAYER_BAR_4V4, {
      R1: { blood: 3, init_blood: 9, name: "1", avatar: "" },
    });
    _c_.emit(EVENT_PLAYER_BAR_4V4, {
      L1: { blood: 3, init_blood: 9, name: "1", avatar: "" },
    });
  }
}
