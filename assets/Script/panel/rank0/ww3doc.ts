    ...
    if (rec) {
        ...
        for (let p of team_R) {
            if (p.name == player_name_R) {
                ...
                if (blood < 0)
                    blood = 0
                if (blood > rightPlayer.init_blood)
                    blood = rightPlayer.init_blood
                rank0.blood_bar_R.y = rank0.BAR_INIT_Y_R - (1 - blood / rightPlayer.init_blood) * rank0.BAR_HEIGHT
